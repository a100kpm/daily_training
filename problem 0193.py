...
and then buy it at 4 dollars and sell it at 10 dollars. Since we did two transactions, 
...
def best_profit(stock,fee,action="buy"):
    if action=="buy":
        if len(stock)<=1:
            return 0
        
        else:
            return max(best_profit(stock[1:],fee,action="buy"),
                       best_profit(stock[1:],fee,action="sell")-stock[0]-fee
                       )
        ...
        if len(stock)==0:
            ...
            return max(best_profit(stock[1:],fee,action="sell"),
                       best_profit(stock[1:],fee,action="buy")+stock[0]
                        )
